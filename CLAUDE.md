...
The frontend's navigation is two top-level sibling menus (Ant Design `Menu`, `frontend/src/shared/navigation/`, rendered by `App.tsx`): **"کاربر"** (User) — **داشبورد** (Dashboard — the default/landing page: a read-only "my tasks" tab (assigned to me, created by me, OR referred to me — no add/edit) plus an ended-*and*-expiring-soon contracts tab), **اطلاعات پایه** (Basic Info — Staff, Positions, Customer Categories, Activity Fields), **امور مشتریان** (Customer Affairs — Customers), and **انجام کار** (Do Task — the full-featured Tasks page, scoped to the current user's own tasks) — and **"ابزار"** (Tools) — currently just **تنظیمات** (Settings). Future top-level menus for other roles are further siblings of these two, not nested under either. Submenus start collapsed except the top-level groups themselves (only one tap/click to find anything, especially on mobile). The nav is responsive per the standard mobile pattern — a persistent sidebar (`Sider`) on desktop, collapsing below 768px (CSS media query, not JS breakpoint detection) into a hamburger button that opens the same menu in an overlay `Drawer`, which auto-closes on selection — since mobile is a firm requirement (see below), not an afterthought.
